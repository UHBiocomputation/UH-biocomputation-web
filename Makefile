...
ftp_upload: publish
	lftp ftp://$(FTP_USER)@$(FTP_HOST) -p $(FTP_PORT) -e "set ftp:ssl-force on; set ftp:ssl-protect-data on; set ssl:verify-certificate no; mirror -R --delete --parallel=3 --ignore-time --exclude cgi-bin/ --exclude .htaccess $(OUTPUTDIR) $(FTP_TARGET_DIR) ; quit"
