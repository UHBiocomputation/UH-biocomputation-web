...
post_category = ":category: Seminars#!/usr/bin/env python
...
message_subject = (
    f"[Journal Club] - {author} - {title} - {formated_date} at {seminar_time} - 2J124 & online"
    + newline
)
...
paragraph1 += (
    f'She/He will talk about the paper entitled "{title}". For more information, please see the abstract below.'
    + newline
)
zoom_notification1 = (
...
reminder_part1 = "A reminder of the next three Journal Club speakers:" + newline
...
# DONE: zoom link may be loaded from a local file" + newline
post_tags = f":tags: {paper_tags[:-1]}" + newline
full_slug = full_file_name.split(".")[0]
full_slug = full_slug.split("/")[-1]
slug = "-".join(full_slug.split("-")[1:])
